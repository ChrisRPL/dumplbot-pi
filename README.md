...
* `partial` workspaces + repo attach. Workspace APIs/state, repo attach, mock UI switching, and renderer workspace selectors landed; broader workspace-local history/project storage polish is still pending.
* `partial` skill packs + optional integrations. Skill schema/loading/policy landed, plus per-workspace defaults, focused job skill editing, and renderer skill selectors; richer integrations are still pending.
