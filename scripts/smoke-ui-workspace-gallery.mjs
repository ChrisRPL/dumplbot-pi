...
const runSmoke = async () => {
  const outputDir = await mkdtemp(join(tmpdir(), "dumplbot-ui-workspace-gallery-"));

  try {
    const result = spawnSync(
      "python3",
      ["apps/ui/dumpl_ui.py", "--preview-workspace-gallery", outputDir],
      {
        cwd: process.cwd(),
        env: process.env,
        encoding: "utf8",
    ...
    assert(result.status === 0, "expected preview workspace gallery command to return 0");
    assert(result.stdout.includes("Workspace gallery saved"), "expected workspace gallery success output");

    for (const fileName of [
      "workspace-summary.png",
      "workspace-detail.png",
      "workspace-history.png",
      "workspace-files.png",
      "workspace-file.png",
    ]) {
    ...
    console.log("ui workspace gallery smoke ok");
  } finally {
