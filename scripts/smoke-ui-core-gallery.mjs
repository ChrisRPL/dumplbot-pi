...
const runSmoke = async () => {
  const outputDir = await mkdtemp(join(tmpdir(), "dumplbot-ui-core-gallery-"));

  try {
    const result = spawnSync(
      "python3",
      ["apps/ui/dumpl_ui.py", "--preview-core-gallery", outputDir],
      {
        cwd: process.cwd(),
        env: process.env,
        encoding: "utf8",
    ...
    assert(result.status === 0, "expected preview core gallery command to return 0");
    assert(result.stdout.includes("Core gallery saved"), "expected core gallery success output");
    ...
    console.log("ui core gallery smoke ok");
  } finally {
