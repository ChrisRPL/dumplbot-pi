...
const runSmoke = async () => {
  const outputDir = await mkdtemp(join(tmpdir(), "dumplbot-ui-scheduler-gallery-"));

  try {
    const result = spawnSync(
      "python3",
      ["apps/ui/dumpl_ui.py", "--preview-scheduler-gallery", outputDir],
      {
        cwd: process.cwd(),
        env: process.env,
        encoding: "utf8",
    ...
    assert(result.status === 0, "expected preview scheduler gallery command to return 0");
    assert(result.stdout.includes("Scheduler gallery saved"), "expected scheduler gallery success output");

    for (const fileName of [
      "scheduler-summary.png",
      "scheduler-detail.png",
      "scheduler-history.png",
    ]) {
    ...
    console.log("ui scheduler gallery smoke ok");
  } finally {
