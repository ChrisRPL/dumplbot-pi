...
const runSmoke = async () => {
  const outputDir = await mkdtemp(join(tmpdir(), "dumplbot-ui-skill-gallery-"));

  try {
    const result = spawnSync(
      "python3",
      ["apps/ui/dumpl_ui.py", "--preview-skill-gallery", outputDir],
      {
        cwd: process.cwd(),
        env: process.env,
        encoding: "utf8",
    ...
    assert(result.status === 0, "expected preview skill gallery command to return 0");
    assert(result.stdout.includes("Skill gallery saved"), "expected skill gallery success output");
    ...
    console.log("ui skill gallery smoke ok");
  } finally {
