...
const runSmoke = async () => {
  const tmpRoot = await mkdtemp(join(tmpdir(), "dumplbot-runner-timeout-smoke-"));
  ...
  await writeFile(
    ...
    [
      ...
      "void readAll().then(() => {",
      "  setTimeout(() => {",
      "    process.stdout.write(JSON.stringify({ type: 'done', summary: 'late' }) + '\\\\n');",
      "  }, 5000);",
      "});",
      "",
    ].join("\n"),
  ...
  try {
    ...
    assert(errorEvents.length === 1, "expected one timeout error event");
    assert(doneEvents.length === 0, "expected no done event after timeout");
    assert(
      errorEvents[0].data?.message === "runner timed out after 1s",
      "expected timeout error message",
    );

    console.log("runner timeout smoke ok");
  } finally {
